(() => {
  ...
  const elements = {
    ...
    backup: $("#backupReminder"), intro: $("#storageIntroNotice"),
    exportNotes: $("#exportNotesButton"), importNotes: $("#importNotesButton"), importInput: $("#importNotesInput"),
  };
  ...
  const STORAGE_INTRO_KEY = "shiye-storage-intro-dismissed-v1";
  let notes = loadNotes();
  ...
  function updateBackupReminder() {
    if (elements.backup) elements.backup.hidden = notes.length === 0;
  }

  function updateStorageIntro() {
    if (!elements.intro) return;
    elements.intro.hidden = localStorage.getItem(STORAGE_INTRO_KEY) === "1";
  }
  ...
  function exportNotes() {
    ...
    downloadJSON(`shiye-notes-backup-${stamp}.json`, NoteStore.createExport(notes));
    showToast(T("Backup exported as JSON", "已导出 JSON 备份"));
  }

  async function importNotesFromFile(file) {
    if (!file) return;
    const text = await file.text();
    const parsed = NoteStore.parseImport(text);
    elements.importInput.value = "";
    if (!parsed.ok) {
      showToast(T("Import failed: invalid Shiye Notes JSON", "导入失败：不是有效的拾页 JSON"));
      return;
    }
    if (!parsed.notes.length) {
      showToast(T("No valid notes found in this file", "这个文件里没有可导入的有效笔记"));
      return;
    }

    let mode = "merge";
    if (notes.length) {
      const merge = confirm(T(
        `Import ${parsed.notes.length} note(s)?\n\nOK: merge with existing notes (recommended)\nCancel: choose whether to replace all notes`,
        `要导入 ${parsed.notes.length} 篇笔记吗？\n\n确定：与现有笔记合并（推荐）\n取消：选择是否覆盖全部笔记`
      ));
      if (!merge) {
        const overwrite = confirm(T(
          "Replace all current notes with this import? This cannot be undone unless you already exported a backup.",
          "是否用导入文件覆盖当前全部笔记？如果没有提前导出备份，此操作无法撤销。"
        ));
        if (!overwrite) {
          showToast(T("Import cancelled", "已取消导入"));
          return;
        }
        mode = "overwrite";
      }
    ...
    notes = mode === "merge" ? NoteStore.mergeNotes(notes, parsed.notes) : NoteStore.sortNotes(parsed.notes);
    activeId = NoteStore.sortNotes(notes)[0]?.id || null;
    if (activeId) localStorage.setItem(ACTIVE_NOTE_KEY, activeId);
    persist();
    renderEditor();
    showToast(mode === "merge" ? T("Notes imported and merged", "笔记已合并导入") : T("Notes imported and replaced", "笔记已覆盖导入"));
  }
  ...
  elements.importNotes.addEventListener("click", () => elements.importInput.click());
  elements.importInput.addEventListener("change", (event) => importNotesFromFile(event.target.files[0]));
  $("#dismissStorageIntro").addEventListener("click", () => {
    localStorage.setItem(STORAGE_INTRO_KEY, "1");
    updateStorageIntro();
  });
