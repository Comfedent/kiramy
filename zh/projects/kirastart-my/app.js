...
function savePlan() { localStorage.setItem('kirastart-plan', JSON.stringify(getValues())); toast(T('Plan saved on this device','计划已保存在此设备')); }
function loadPlan() {
  const raw = localStorage.getItem('kirastart-plan');
  if (!raw) { toast(T('No saved plan yet — let’s make one','还没有已保存的计划——现在开始制定')); startPlanner(); return; }
  try { const data = JSON.parse(raw); Object.entries(data).forEach(([key,value]) => { if (!form.elements[key]) return; if (form.elements[key].type === 'checkbox') form.elements[key].checked = Boolean(value); else form.elements[key].value = value; }); startPlanner(); toast(T('Saved plan loaded','已加载保存的计划')); } catch { localStorage.removeItem('kirastart-plan'); startPlanner(); }
}
...
$('#resetButton').addEventListener('click', () => { form.reset(); localStorage.removeItem('kirastart-plan'); currentStep=1; updateStep(); showView('planner'); toast(T('Fresh plan ready','新计划已准备好')); });
$('#copySummaryButton').addEventListener('click', async () => {
  ...
  try { await navigator.clipboard.writeText(text); toast(T('Summary copied','摘要已复制')); } catch { toast(T('Copy is not available in this browser','当前浏览器无法复制')); }
});
